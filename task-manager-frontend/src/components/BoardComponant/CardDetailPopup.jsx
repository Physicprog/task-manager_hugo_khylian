...
export default function CardDetailPopup({ card, column, onClose, onUpdate, onDelete }) {
    ...
    <div className="flex items-center gap-2">
        <input type="checkbox" checked={formData.favorite} onChange={e => setFormData(Object.assign({}, formData, { favorite: e.target.checked }))} disabled={isLoading}/> 
    </div>
            ...
            <div className="bg-secondary p-6 rounded-lg w-[500px] max-w-[90%] border border-accent1 shadow-lg" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div>
                        ...
                        <input type="text" value={formData.title} onChange={(e) => setFormData(Object.assign({}, formData, { title: e.target.value }))}
                            className="w-full px-3 py-2 bg-surface text-text rounded outline-none focus:ring-2 focus:ring-accent1"
                            disabled={isLoading} placeholder="Card title"/>
                    </div>

                    <div>
                        <label className="block text-text text-sm mb-1">Description</label>
                        <textarea value={formData.description} onChange={(e) => setFormData(Object.assign({}, formData, { description: e.target.value }))} className="w-full px-3 py-2 bg-surface text-text rounded outline-none focus:ring-2 focus:ring-accent1 resize-none"disabled={isLoading} placeholder="Card description"/>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="block text-text text-sm mb-1">Priority</label>
                            <select value={formData.priority} onChange={(e) => setFormData(Object.assign({}, formData, { priority: e.target.value }))}
                        ...
                        <div>
                            ...
                            <select value={formData.label}
                                onChange={(e) => setFormData(Object.assign({}, formData, { label: e.target.value }))} className="w-full px-3 py-2 bg-surface text-text rounded outline-none focus:ring-2 focus:ring-accent1" disabled={isLoading}>
                                <option value="">No label</option>
                                {Object.keys(labelColors).map(label => (<option key={label} value={label}>{label}</option>))}
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2 mt-4">
                        ...
                        <button onClick={handleDelete} disabled={isLoading} className="px-4 py-2 bg-red-500 text-white rounded hover:opacity-90 disabled:opacity-50">
                            Delete
                        </button>
                    </div>
                </div>
